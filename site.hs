  ...
  $ do
    ...
    create ["archive.html"] $ do
      ...
      compile $ do
        ...
        let archiveCtx =
              listField "postGroup" postGroupCtx (return postsGroup)
                `mappend` constField "title" (feedTitle feedConfig ++ " : Archive")
                `mappend` defaultContext
        makeItem ""
      ...
      compile $ do
        ...
        let indexCtx =
              listField "posts" postCtx (return posts)
                `mappend` constField "title" (feedTitle feedConfig)
                `mappend` defaultContext
        getResourceBody
...
postGroupCtx =
  field "month" (return . month . fst . itemBody)
    `mappend` listFieldWith "posts" postCtx (return . snd . itemBody)
